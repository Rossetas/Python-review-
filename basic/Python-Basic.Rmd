---
title: "Python"
author: "Marco Vásquez Ovares  (https://github.com/Rossetas)"
...
##### **Ejercicio #1** Definir una función **num_max( )** que reciba como argumento dos números y devuelva el mayor de ellos. (Es cierto que Python tiene una funcion *max( )* pero hacerla nosotros mismos es un buen ejercicio para iniciar).
```{python}
def num_max(a=0, b=0):
  if (a > b):
    return a
...
##### **Ejercicio #3** Definir una función que calcula la longitud de una lista o de una cadena dada. (Es cierto que Python tiene la función *len( )* incorporada, pero escribirla por nosotros mismos resulta en un muy buen ejercicio)
```{python}
...
##### **Ejercicio #5** Escribir una función **sum( )** y una funcion **mult( )** que suman y multiplican respectivamente todos los números de una lista. Por ejemplo *sum([1,2,3,4])* debe devolver 10 y *mult([1,2,3,4])* debe devolver 24
```{python}
def sum(lista_sum):
  result_sum=0
  for i in lista_sum:
...
##### **Ejercicio #6** Escribir una función **inversa( )** que calcule la forma inversa de una cadena. Por ejemplo la cadena *"estoy probando"* debe devolver la cadena *"odnaborp yptse"*
```{python}
def inversa(cadena):
  longitud = len(cadena)
  new_cadena = str()
...
##### **Ejercicio #7** Escribir una función **palindromo( )** que reconoce palíndromos (es decir, palabras que tienen el mismo aspecto escritas invertidas), por ejemplo *palindromo("radar")* debe devolver **True**
```{python}
...
##### **Ejercicio #9** Definir una función **generar_n_caracteres( )** que tome un entero **n** y devuelva el carácter multiplicado por n. Por ejemplo *generar_n_caracteres(5, 'x')* debe devolver 
##### "xxxxx"
```{python}
...
##### **Ejercicio #10** Definir una función **histograma( )** que tome una lista de números enteros e imprima un histograma en la pantalla. Ejemplo *histograma([4,9,7])*
